<template>
  <div class="kb-panel">
    <div class="kb-toolbar">
      <n-space>
        <n-button type="primary" :loading="loading" @click="refresh">
          <template #icon><span>🔄</span></template>
          刷新列表
        </n-button>
        <n-button type="warning" :loading="rebuilding" @click="confirmRebuild">
          <template #icon><span>🔧</span></template>
          重建索引
        </n-button>
      </n-space>
      <span class="kb-count">共 {{ docs.length }} 项</span>
    </div>
      ...
      <div v-else class="kb-list">
        ...
        >
          ...
          <div class="kb-meta">
            ...
            <div class="kb-sub">
              <n-tag size="tiny" :type="d.source === 'builtin' ? 'info' : 'default'" :bordered="false">
                {{ d.source === 'builtin' ? '内置' : '已上传' }}
              </n-tag>
...
import { NSpace, NButton, NSpin, NEmpty, NTag, NAlert, useDialog, useMessage } from 'naive-ui'
import { listDocs, deleteDoc, rebuildIndex } from '../../api/ragApi.js'

export default {
  name: 'KbPanel',
  components: { NSpace, NButton, NSpin, NEmpty, NTag, NAlert },
  setup() {
    const docs = ref([])
...
.kb-count { font-size: 13px; color: var(--color-text-faint); }
...
.kb-row.kb-builtin { background: rgba(0, 123, 255, 0.04); }
.kb-icon { font-size: 22px; flex-shrink: 0; }
